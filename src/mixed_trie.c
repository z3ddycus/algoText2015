...
Trie newTrie() {
  ...
  result->first = calloc(UCHAR_MAX, sizeof(SortedList));
  FAIL_IF(result->first == NULL, "trie->first : malloc failed");

  result->trans = calloc(INIT_CAPACITY, sizeof(SortedList));
...
size_t getNext(Trie t, unsigned char c, size_t cur) {
  ...
  size_t next = 0;
  SortedList l = t->trans[cur];
  if (l == NULL || (next = get(l, c)) == GET_ERR) {
    if (cur == 0) {
      return 0;
    }
...
size_t insertLetter(Trie t, unsigned char c, size_t last) {
  if (last == 0) {
    ...
    t->next++;
    #ifdef DEBUG_TRIE
      fprintf(stderr, "\tnew node at index %4zu  from root node (character %20c)\n",
        t->next, c);
    #endif
    size_t next = t->next;
    if (t->next >= t->max) {
  ...
  if (t->trans[last] == NULL) {
    t->trans[last] = newList();
  }
  size_t next = get(t->trans[last], c);
  if (next == GET_ERR) {
    // Nouvelle transition
    t->next++;
    if (t->next >= t->max) {
